...
def _parse_request_data(request):
    ...
    if request.FILES:
        ...
        try:
            ...
            return (
                ...
                None,
                JsonResponse(
                    {"error": f"Unable to read uploaded file: {e}"}, status=400
                ),
            )
    ...
    if not binary_data:
        ...
        return (
            ...
            None,
            None,
            JsonResponse({"error": "No binary data in request"}, status=400),
        )
...
def _try_deserialize_with_protocol(data, prot_cls, packet_ttypes):
    ...
    """
    logger.debug(
        "Attempting deserialize: protocol=%s, data_len=%d",
    ...
    try:
        ...
        pkt.read(iprot)
        logger.debug(
            "Deserialization attempt successful for protocol=%s",
            getattr(prot_cls, "__name__", str(prot_cls)),
        )
        return pkt
    except Exception as e:
        logger.debug(
            "Deserialization attempt failed for protocol=%s: %s",
            getattr(prot_cls, "__name__", str(prot_cls)),
            repr(e),
            exc_info=True,
        )
...
def _try_strip_frame_and_deserialize(
    data, frame_prefix_len, byteorder, prot_cls, packet_ttypes, errors, strategy_name
):
    """
    ...
    if len(data) <= frame_prefix_len:
        logger.debug(
            "Data too short to contain frame prefix: data_len=%d, prefix_len=%d",
            len(data),
            frame_prefix_len,
        )
        return None
    ...
    try:
        ...
        logger.debug(
            "Parsed frame length=%d from prefix_len=%d (byteorder=%s) for strategy=%s",
            frame_len,
            frame_prefix_len,
            byteorder,
            strategy_name,
        ...
        if (
            frame_len == len(data) - frame_prefix_len
            or frame_len <= len(data) - frame_prefix_len
        ):
            stripped = (
                data[frame_prefix_len : frame_prefix_len + frame_len]
                if frame_len <= len(data) - frame_prefix_len
                else data[frame_prefix_len:]
            )
...
def _deserialize_packet(binary_data, packet_ttypes):
    ...
    res = _try_deserialize_with_protocol(
        binary_data, TCompactProtocol.TCompactProtocol, packet_ttypes
    )
    if isinstance(res, packet_ttypes.Packet):
        logger.info(
            "Deserialization succeeded using strategy 'compact-as-is' (protocol=%s)",
            getattr(
                TCompactProtocol.TCompactProtocol,
                "__name__",
                str(TCompactProtocol.TCompactProtocol),
            ),
        )
    ...
    res = _try_deserialize_with_protocol(
        binary_data, TBinaryProtocol.TBinaryProtocol, packet_ttypes
    )
    if isinstance(res, packet_ttypes.Packet):
        logger.info(
            "Deserialization succeeded using strategy 'binary-as-is' (protocol=%s)",
            getattr(
                TBinaryProtocol.TBinaryProtocol,
                "__name__",
                str(TBinaryProtocol.TBinaryProtocol),
            ),
        )
...
def _field_meta_to_dict(fm):
    ...
    if getattr(field, "ether", None) is not None:
        ...
        ether_type_hex = hex(unsigned_i16_to_u16(eth.type))
    ...
    if getattr(field, "vlan", None) is not None:
        ...
        vlan_type_hex = hex(unsigned_i16_to_u16(vlan.type))
        d["vlan"] = {"vlan": vlan.vlan, "type": vlan_type_hex}

    if getattr(field, "ipv4", None) is not None:
        ip4 = field.ipv4
        d["ipv4"] = {
            "src_ip": hex(unsigned_i32_to_u32(ip4.src_ip)),
            "dst_ip": hex(unsigned_i32_to_u32(ip4.dst_ip)),
        }
...
def unsigned_i32_to_u32(signed_i32_value):
    # 32位的最大掩码：0xFFFFFFFF (即 2**32 - 1)
    MASK_32_BIT = 0xFFFFFFFF

    # 检查值是否为负数（即其最高位是否为1）
    if signed_i32_value < 0:
        # 如果是负数，则用位掩码进行 "反符号扩展"
        # 相当于加上 2^32，将其提升到正数范围
        unsigned_value = signed_i32_value & MASK_32_BIT
        return unsigned_value
    else:
        # 如果是正数，直接返回
        return signed_i32_value


def unsigned_i16_to_u16(signed_i16_value: int) -> int:
    """
    将 Thrift i16 字段（可能包含 U16 值）在 Python 中还原为正确的无符号 16 位整数。

    参数:
        signed_i16_value: 从 Thrift 接收到的 i16 值 (Python int 类型)。

    返回:
        还原后的 U16 值 (Python int 类型)。
    """
    # 16 位的最大掩码：0xFFFF (即 2**16 - 1)
    MASK_16_BIT = 0xFFFF

    # 检查值是否为负数（即其第 15 位是否为 1）
    if signed_i16_value < 0:
        # 如果是负数，则用位掩码进行 "反符号扩展"
        # 相当于加上 2^16，将其提升到正数范围
        unsigned_value = signed_i16_value & MASK_16_BIT
        return unsigned_value
    else:
        # 如果是正数，直接返回
        return signed_i16_value
...
def stream_backend_packet_send(request):
    ...
    try:
        # Parse request data
        binary_data, received_filename, parsed_fields, error_response = (
        ...
        try:
            ...
            return JsonResponse(
                {"error": f"Failed importing generated thrift packet types: {e}"},
                status=500,
            )
